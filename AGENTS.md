...
阶段 0–7 已完成（审计 / 环境 / 数据治理 / Tokenizer / TinyStories 快速预训练 / Wikitext 正式预训练 / Qwen3 CPT / SFT，见 docs/00 各阶段补充）。2026-08-10 定位升级：项目是 LLM 全链路教学项目，主线教学模型为 5090 单卡可承载的尽可能大的从零预训练模型（100M–200M 级，D≈20N × 时间预算决策，见 docs/01 §3.1），小模型资产保留为教程对比项，Qwen3-0.6B 为强基座对照线；阶段编号重排（RM→10、DPO→11、GRPO→12、多模态→13、评测→14、量化→15、部署→16、验收→17）。

...
阶段 0–7 已完成（审计 / 环境 / 数据治理 / Tokenizer / TinyStories 快速预训练 / Wikitext 正式预训练 / Qwen3 CPT / SFT，见 docs/00 各阶段补充）。2026-08-10 定位升级：项目是 LLM 全链路教学项目，主线教学模型规模按"尽可能大"原则结合实测数据决策（minimind_dataset ~1.4B tokens → 严格 D≈20N → **~70M**，见 docs/01 §3.1），小模型资产保留为教程对比项，Qwen3-0.6B 为强基座对照线；阶段编号重排（RM→10、DPO→11、GRPO→12、多模态→13、评测→14、量化→15、部署→16、验收→17）。

当前执行阶段 8：主线从零预训练（minimind_dataset 数据治理、规模决策、训练、与小模型对照；任务与验收见 docs/02 阶段 8）。

当前不编写训练代码，不启动 GPU 训练或推理，不修改服务器系统配置。模型、数据和环境准备只在用户明确授权时执行；未授权的后续训练、评测、量化和部署只记录，不顺手实现。完成条件是阶段 8 任务与验收逐项达成、文档一致、服务器资产事实可追溯。
...
- 优先单卡；
- 两卡只用于明确多卡学习；
- 单次大显存 GPU 任务默认不超过 8 小时；
- 时间可放宽，但小于 1 天（24 小时）是硬上限；

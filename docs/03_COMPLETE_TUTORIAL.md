...
**主线模型（2026-08-10 定位升级）**：本项目的主线教学模型是 5090 单卡可承载的**尽可能大**的从零预训练模型（100M–200M 级，规模决策方法见 docs/01 §3.1 与 docs/06 Q21），贯穿后续 SFT、RM、DPO、GRPO、统一评测、量化、部署全链路；TinyStories 18.1M、Wikitext 5.32M/26.8M 等小模型资产保留为教程中的规模/容量对比项；Qwen3-0.6B 为强基座对照线。
...
先用 TinyStories 验证实现（阶段 4，已完成），再用 Wikitext 做小模型正式实验（阶段 5，已完成），最后在治理后的主线语料（minimind_dataset，阶段 8）上训练 100M–200M 级主线教学模型。

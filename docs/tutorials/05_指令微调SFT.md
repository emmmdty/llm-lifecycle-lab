...
### 1.1 为什么用 Qwen3-0.6B-Base？（承接教程 04）

模型选型理由在教程 04 第 1.1 节已完整说明（单卡可训、双语覆盖、Base 无后训练污染、生态成熟）。这里补充两点 SFT 专属的考虑：

- **0.6B 是"LoRA 指令微调"的最小可行规模**：实验 B/C 用 rank-8 adapter（2.29M 可训练参数）在 222 步内把 held-out assistant ppl 从 8.40 降到 6.20/6.58，且生成质量明显提升——同样的指令数据给 18.1M 小模型（实验 A）会出现生成退化（第 5.4 节）。这说明 0.6B 是这个任务规模下"能学到行为格式"的规模下限附近；
- **与官方 Instruct 版形成对照**：`Qwen3-0.6B`（Instruct 版）本身就是官方 SFT 产物，用它对比可以直观看到"别人家 SFT 的效果天花板"，但本项目主线始终从 Base 出发，保证训练链路每一步都是自己完成的。

**关键澄清（与教程 04 相同）：Qwen3-0.6B-Base 原始权重从未被修改。** 实验 B/C 只训练 LoRA adapter；QLoRA 的 NF4 基座从磁盘重建、只读使用。第 4.2 节的 `merge_and_unload` 全部发生在**内存副本**上，合并结果可另存为新目录（`--merged-dir`），`models/Qwen3-0.6B-Base/` 自阶段 0 下载后从未写入。SFT 的训练产物只有 checkpoint（adapter + 优化器 + RNG）与 adapter 目录。

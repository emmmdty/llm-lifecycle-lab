...
| 能力 | 任务级评测、生成人工对照 | 阶段 14 统一评测 + samples.jsonl |
...
用 distinct 4-gram ratio 量化（1.0=无重复，0=完全重复）：TinyStories 模型 1.0，Wikitext 模型 0.59–0.63。小模型 + 百科分布（词汇稀疏、长尾词多）在纯自回归采样下容易陷入重复回路——这是预训练模型评测中"loss 低≠生成好"的典型例证，也解释了为什么阶段 14 需要任务级评测。

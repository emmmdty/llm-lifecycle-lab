...
按 8h 默认预算 + 实测吞吐推算，单卡 5090 每个任务 compute-optimal（D=20N）的上限约 **128M 参数 / 2.6B tokens**；放宽到 24h 硬上限时约 **200M 参数 / 4B tokens**（计算：`N = sqrt(时间×实测吞吐/120)`，实测 ~66 TFLOPS；2026-08-10 修正，原记录"875M/17.5B"与 §4.3"1B/20B ≈ 21 天"矛盾，错约 10 倍，勘误记录见 docs/00）。64M 规模（minimind-3 级别，~5 亿 token）只需约 50 分钟。**2026-08-10 用户决策：主线教学模型按"尽可能大"规划，目标 ~200M / 4B token / 单次 ~20h（放宽记录），保守回落 ~128M / 8h（Q21）。**
...
> 2026-08-10 定位更新：minimind 从"社区参照系"升级为"同规模级对比项 + 主线预训练语料候选"（Q21/Q25）。本项目不复制其训练配方，而是把它的语料（minimind_dataset）重新治理后按 D≈20N 决策训练主线模型。
...
- Q7：序列长度与位置编码（512 learned-absolute vs 1024/RoPE）对生成质量的影响。**（阶段 5 未验证：512→1024 同时换了语料与词表，对照被混淆；留待阶段 14 或专门实验）**

**评价侧**
- Q8：效果评价升级——仅 val ppl 不够。需补充：val-train gap（过拟合诊断）、生成重复率/多样性、固定 prompt 人工对照（已有 samples.jsonl 基础）、任务级评测（阶段 14 统一评测；此前可引入轻量过渡评测）。**✅ 已验证（阶段 5 + 阶段 6）：val-train gap 已纳入 metrics（全部运行 gap>0 且小：0.017–0.147）；生成多样性（distinct 4-gram ratio）已纳入（TinyStories 1.0、Wikitext 0.59–0.63、阶段 6 CPT 0.632→0.655）；阶段 6 固定 3 prompt（法律/中文日常/英文）Base vs CPT 生成对照入 reports/cpt-compare-formal.json；任务级评测留待阶段 14。**
...
| 能力 | 任务级评测、生成人工对照 | 阶段 14 + 生成样本（已有，Q8 ✅ 生成多样性量化） |
...
| Q7 | 序列长度与位置编码对比 | 阶段 5 | 未验证（512→1024 换语料/词表，对照混淆；留待阶段 14 或专门实验） |
| Q8 | 效果评价升级（gap/多样性/过渡评测） | 阶段 5/12 | ✅ 已验证（阶段 5：val-train gap + 4-gram 多样性纳入；阶段 6 补充：固定 3 prompt Base vs CPT 生成对照 + 多样性 0.632→0.655；任务级评测留待阶段 14） |
...
| Q21 | 主线预训练规模决策（minimind_dataset 治理后 token 预算 → D≈20N → 时间预算校验；候选 100M–200M，默认按"尽可能大"规划 ~200M，回落 ~128M） | 阶段 8 | 待验证（2026-08-10 登记；决策框架见 docs/01 §3.1 与 docs/00 补充） |
...
| Q26 | minimind_dataset 治理（许可证核对、split、去重、token 统计、与 minimind 原始用法差异） | 阶段 8 | 待验证 |
...
## 9. 阶段 8 主线预训练决策登记（2026-08-10 定位升级）

### Q21：主线预训练规模决策（待验证，阶段 8 执行）

用户决策（2026-08-10）：主线教学模型 = 5090 单卡可承载的尽可能大的从零预训练模型；64M 只是示例，不是上限。决策方法（沿用 Q1/Q2 已验证框架）：

```text
1. 治理 minimind_dataset pretrain，实测 token 总数 D（候选 1.2GB mini / 10GB 主线）；
2. N = D/20（D≈20N，compute-optimal）；
3. 校验 6·N·D / 实测吞吐 ≤ 时间预算（默认 8h；放宽至 <24h 需记录理由）；
4. 兜底：D 不足则降规模匹配，或数据受限记录（minimind 先例 t/p≈7.8）。
```

规划值：目标 ~200M / ~4B token / 单次 ~20h（放宽理由 = 用户"尽可能大"决策）；回落方案 ~128M / ~2.6B token / 8h。上限事实（修正）：8h → ~128M，24h → ~200M（原"875M"记录为错误，勘误见 docs/00 2026-08-10 补充）。

### Q26：minimind_dataset 治理（待验证，阶段 8 执行）

- 下载前盘点大小、文件、许可证（Apache-2.0）与 split；
- 按阶段 2 流程：schema 检查 → 去重 → 有界抽样 → 按行/document 分组切分 held-out → token 流 → manifest；
- 记录与 minimind 原始用法差异（minimind 直接当自回归文本流；我们治理 + 切分 + manifest）；
- 硬盘预算：raw ≤10GB + token 流 ≤16GB + checkpoint ≤16 个 ≈ 合计 ≤32GB。

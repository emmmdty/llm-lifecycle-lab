...
def write_stream(stream: Iterator[int], path: Path, batch_size: int = _STREAM_BATCH) -> int:
    """Append tokens to an int32 binary stream file; returns total token count."""
...
def read_stream(path: Path) -> list[int]:
    """Read a small int32 stream fully into memory (tests / small validation)."""
    from array import array
    ...
    with path.open("rb") as fh:
        data.fromfile(fh, size // 4)
    return list(data)


def open_stream_memmap(path: Path):
    """Memory-map an int32 stream for low-memory random block access."""
    import numpy as np

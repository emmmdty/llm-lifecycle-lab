...
DATASETS: dict[str, DatasetSpec] = {
    ...
    "chartqa": DatasetSpec(
        name="chartqa",
        license="snapshot README 未标注，待确认",
        revision="lmms-lab/ChartQA test snapshot 2026-07-29",
        upstream="lmms-lab/ChartQA",
        reader="parquet",
        transform="chartqa",
        split_strategy="official",
        official_files={
            "test": ("chartqa/data/test-*.parquet",),
        },
        projection=("type", "question", "answer"),
        seed=42,
        notes=(
            "当前 snapshot 仅有 test split，train 缺失，阶段 11 前需重新下载。",
            "文本字段进 parquet；image 保留在 raw，不进 processed。",
        ),
    ),
